...
class MessagePassing(Layer):
    ...
    **Arguments**:
    ...
    def propagate(self, x, a, e, a3, e3, **kwargs):
        ...
        self.hyperindex_h3 = a3.indices[:, 0]

        # Message
        msg_kwargs = self.get_kwargs(x, a, e, a3, e3, self.msg_signature, kwargs)
        messages = self.message(x, **msg_kwargs)

        # Aggregate
        agg_kwargs = self.get_kwargs(x, a, e, a3, e3, self.agg_signature, kwargs)
        embeddings = self.aggregate(messages, **agg_kwargs)

        # Update
        upd_kwargs = self.get_kwargs(x, a, e, a3, e3, self.upd_signature, kwargs)
    ...
    def get_hyper2(self, e):
        return tf.gather(e, self.hyperindex_h2, axis=-2)

    def get_hyper3(self, e):
        return tf.gather(e, self.hyperindex_h3, axis=-2)

    def get_kwargs(self, x, a, e, a3, e3, signature, kwargs):
        output = {}
    ...
    def get_inputs(inputs):
        ...
        if len(inputs) == 5:
            ...
            assert K.ndim(e3) in (2, 3), "E3 must have rank 2 or 3"       
        else:
            raise ValueError(
                "Expected 5 inputs tensors (X, A, E, A3, E3), got {}.".format(len(inputs))
            )
        ...
        assert K.is_sparse(a3), "A3 must be a SparseTensor"
        assert K.ndim(a3) == 2, "A3 must have rank 2"
